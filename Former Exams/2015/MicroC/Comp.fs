...
let allocate (kind : int -> var) (typ, x) (varEnv : varEnv) : varEnv * instr list =
    ...
    | TypA (t, Some i) ->
      ...
      (newEnv, code)
    | TypR (t, from, step, max) ->
      let rec lengthCal value acc = if value < max then lengthCal (value + step) (acc+1) else acc 
      let length = lengthCal from 0
      let newEnv = ((x, (kind (fdepth+length), typ)) :: env, fdepth+length+1)
      let rec doSteps value (acc:instr list) : instr list = 
        match value with 
        | x when x < max -> doSteps (x+step) (acc @ [CSTI x])
        | _ -> acc
      let code = doSteps from [] @ [GETSP; CSTI (length-1); SUB]
      (newEnv, code)
    | _ ->
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) : instr list =
    ...
    | Prim1(ope, e1) ->
      let instr x = match x with
                  ...
                  | _          -> raise (Failure "Not array inside arrlen"))
      cExpr e1 varEnv funEnv
      @ (match ope with
         | "!"      -> [NOT]
         | "printi" -> [PRINTI]
         | "printc" -> [PRINTC]
         | "arrlen" -> (instr e1) @ [ARRLEN; SWAP; INCSP -1]
